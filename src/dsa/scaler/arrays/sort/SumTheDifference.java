...
public class SumTheDifference {
    ...
    public int solve(ArrayList<Integer> A) {
        ...
        for(int i=0;i<A.size();i++){
            int contribution = ((1 << i) - (1 << (A.size()-i-1)));
            if(contribution < 0){
                contribution = -1*contribution;
                total = (total%MOD - (A.get(i)%MOD * contribution%MOD)%MOD + MOD)%MOD;
            }else{
                total = (total%MOD + (A.get(i)%MOD * contribution%MOD)%MOD)%MOD;
            }
        }
        return total;
    }
